...
// .wrangler/tmp/bundle-LHsYPB/checked-fetch.js
...
app.patch("/api/admin/bookings/:id", async (c) => {
  ...
  try {
    ...
    const existingBooking = await env.DB.prepare(`SELECT * FROM bookings WHERE id = ?`).bind(bookingId).first();
    if (!existingBooking) return c.text("Booking Not Found", 404);
    const statusChangedToConfirmed = existingBooking.status === "PENDING" && updateData.status === "CONFIRMED";
    const statusChangedToCancelled = existingBooking.status !== "CANCELLED" && updateData.status === "CANCELLED";
    const updateQuery = `
            UPDATE bookings 
...
var src_default = app;
async function sendAdminNotification(bookingData, env) {
  ...
  try {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: ["roadroamcarrentals@gmail.com"],
        // test address
        subject,
        html: body
      })
...
__name(sendAdminNotification, "sendAdminNotification");
async function sendUserConfirmation(bookingData, env) {
...
__name(sendUserConfirmation, "sendUserConfirmation");
async function sendUserCancellation(bookingData, env) {
...
__name(sendUserCancellation, "sendUserCancellation");
...
// .wrangler/tmp/bundle-LHsYPB/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-LHsYPB/middleware-loader.entry.ts
