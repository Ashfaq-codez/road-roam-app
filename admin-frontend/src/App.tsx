...
function AdminDashboard() {
  ...
  // --- NEW STATE: Tracks if the page was restored from cache ---
  ...
  // 3. LIFECYCLE HOOK (for initial data fetch)
  useEffect(() => {
    fetchBookings();
  }, [fetchBookings]); 
...
useEffect(() => {
    ...
    const handlePageShow = (event: PageTransitionEvent) => {
        ...
        if (event.persisted) { 
            // 1. Immediately set state to block rendering of sensitive data
            setIsCacheRestored(true); 
            
            // 2. Force a hard reload. This clears the cached memory and forces the browser
    ...
    <>
     {/* --- NEW GLOBAL CONFIRMATION MODAL (ENHANCED STYLING) --- */}
      {showConfirmModal && (
        ...
        >
          <div 
            className="bg-gray-800 rounded-xl shadow-2xl p-8 max-w-md w-full border border-gray-700 transform scale-95 animate-scale-in"
          ...
          >
            ...
            <div className="flex justify-end space-x-3">
              ...
              >
                {showConfirmModal.type === 'warning' ? 'Proceed' : 'OK'}
              </button>
            </div>
          </div>
        </div>
      ...
      {isCacheRestored ? (
        <div className="min-h-screen bg-gray-900 flex items-center justify-center">
            <h1 className="text-xl font-semibold text-gray-500">Checking session status...</h1>
        </div>
      ) : (
